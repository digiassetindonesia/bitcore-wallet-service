...
EmailService.prototype.start = function(opts, cb) {
  ...
  self.defaultUnit = opts.emailOpts.defaultUnit || 'cps';
  ...
  async.parallel([
    ...
    function(done) {
      self.mailer = opts.mailer || sgMail;
      done();
    },
  ], function(err) {
    if (err) {
  ...
  self.storage.fetchWallet(notification.walletId, function(err, wallet) {
    ...
    self.storage.fetchPreferences(notification.walletId, null, function(err, preferences) {
      ...
      var recipients = _.compact(_.map(preferences, function(p) {
        ...
        if (self.availableLanguages.includes(p.language)) {
        ...
        var unit;
        var coins = ["btc", "cps", "bit", "bits"];
        if (coins.includes(wallet.coin.toLowerCase())) {
          var unit = wallet.coin;
        } else {
