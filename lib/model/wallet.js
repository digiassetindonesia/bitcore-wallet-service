...
var DigiassetBitcore = require('digiasset-bitcore-lib');
...
Wallet.prototype.createAddresses = function(isChange) {
  ...
  if (self.coin === 'cps' && !self.singleAddress) {
...
Wallet.prototype.getColdStakingAddresses = function(spendAddress) {
  ...
  if (self.coin !== 'cps' || _.isEmpty(self.coldStakingSetup)) {
    return addresses;
  }

  if (self.coldStakingSetup.staking_key.startsWith('ccs') ||
      self.coldStakingSetup.staking_key.startsWith('tccs')) {
    ...
    const xPub = DigiassetBitcore.HDPublicKey(self.coldStakingSetup.staking_key);
